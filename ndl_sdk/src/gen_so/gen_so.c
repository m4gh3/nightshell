...
void list_headers(const char *path, const char *inc_prefix )
{
...
int main()
{
	...
	system("arm-none-eabi-gcc *.o -shared -o ../libcplug.so");
	system("rm *.o");
	...
	for(int i=0; i < 3; i++ )
		list_headers(include_paths[i][0], include_paths[i][1] );
	FILE *pipe = popen("arm-none-eabi-objdump -T ../libcplug.so", "r" );
	char str[256];
